...
@_cdecl("mlx_key_hook")
public func mlx_key_hook_swift(_ winptr:UnsafeRawPointer, _ fctptr:UnsafeMutableRawPointer, _ infoptr:UnsafeMutableRawPointer) -> Int32
{
	let win:MlxWin = _mlx_bridge(ptr:winptr)
	win.addHook(index: 3, fct: fctptr, info: infoptr)
	return (Int32(0));
}


@_cdecl("mlx_mouse_hook")
public func mlx_mouse_hook_swift(_ winptr:UnsafeRawPointer, _ fctptr:UnsafeMutableRawPointer, _ infoptr:UnsafeMutableRawPointer) -> Int32
{
        let win:MlxWin = _mlx_bridge(ptr:winptr)
        win.addHook(index: 4, fct: fctptr, info: infoptr)
        return (Int32(0));
}


@_cdecl("mlx_hook")
public func mlx_hook_swift(_ winptr:UnsafeRawPointer, _ xevent:Int32, _ xmask:Int32, _ fctptr:UnsafeMutableRawPointer, _ infoptr:UnsafeMutableRawPointer) -> Int32
{
        let win:MlxWin = _mlx_bridge(ptr:winptr)
        win.addHook(index: Int(xevent), fct: fctptr, info: infoptr)
        return (Int32(0));
}


@_cdecl("mlx_expose_hook")
public func mlx_expose_hook_swift(_ winptr:UnsafeRawPointer, _ fctptr:UnsafeMutableRawPointer, _ infoptr:UnsafeMutableRawPointer) -> Int32
{
        let win:MlxWin = _mlx_bridge(ptr:winptr)
        win.addHook(index: 12, fct: fctptr, info: infoptr)
        return (Int32(0));
}

@_cdecl("mlx_loop_hook")
public func mlx_loop_hook_swift(_ mlxptr:UnsafeRawPointer, _ fctptr:UnsafeMutableRawPointer, _ infoptr:UnsafeMutableRawPointer) -> Int32
{
	let mlx:MlxMain = _mlx_bridge(ptr:mlxptr)
	mlx.addLoopHook(fctptr, infoptr)
        return (Int32(0));
}
...
@_cdecl("mlx_sync")
public func mlx_sync_swift(_ what:Int32, _ info:UnsafeRawPointer) -> Int32
{
    switch what
    {
	case MLX_SYNC_IMAGE_WRITABLE:
		let img:MlxImg = _mlx_bridge(ptr:info); while img.onGPU > 0 {}
	case MLX_SYNC_WIN_FLUSH_CMD:
		let win:MlxWin = _mlx_bridge(ptr:info); win.flushImages()
	case MLX_SYNC_WIN_CMD_COMPLETED:
	        let win:MlxWin = _mlx_bridge(ptr:info); win.flushImages(); win.waitForGPU()
	default:
		break
    }
    return Int32(0)
